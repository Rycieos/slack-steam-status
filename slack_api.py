...
async def add_user(token: str = Form(...), text: str = Form(...)):
  ...
  return {
    ...
    "text": "<{}/api/slack_auth/{}|Click here> to enable Steam status "
        "syncing".format(HTTP_BASE_URL, steam_id),
  }
...
def auth(steam_id: str):
  return RedirectResponse("https://slack.com/oauth/v2/authorize?"
      ...
      SLACK_APP_ID, SLACK_SCOPE, steam_id, HTTP_BASE_URL + REDIRECT_URI))

# Endpoint for end of oauth dance. Save the user's oauth token
@app.get(REDIRECT_URI, status_code=201)
